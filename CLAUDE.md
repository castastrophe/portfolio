...
Resume, candidate, and experience content is stored in Netlify Blobs under the store name `content` with these keys: `employees`, `experience`, `resume_variants`, `skills`, `resume_variant_skills`.

**Why Blobs instead of a database**: the data is private (cover letters, contact info) and must never be committed to the repository, but still needs to be available at build time without requiring a running database.

**Reading data**: `utilities/data/index.js` exports a `BlobsStore` class that wraps `@netlify/blobs`. In the Netlify build environment, credentials are picked up automatically. For local development, set `NETLIFY_SITE_ID` and `NETLIFY_TOKEN` in `.env`.

**Seeding data**: `utilities/data/seed.js` uploads all content to Netlify Blobs. This file is listed in `.gitignore` (it contains private data) and must never be committed. Run it once after cloning or whenever content changes:

To recreate `utilities/data/seed.js` from a new SQL dump, extract the data and populate the arrays in that file — the structure mirrors the SQL tables with sequential numeric IDs in place of binary UUIDs.
