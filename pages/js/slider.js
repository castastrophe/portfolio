var slider = {
  ...
  getID: (el) => el.attr(this.attr.id),
  isVaryHeight: (parent) =>
    parent.has(this.switcher.slide).attr(this.attr.height) == "variable",
  isCurrent: (slide) => slide.attr(this.attr.state) == "current",
  getAllContent: (items) => items.map((i) => i.id.toString()).get(),
  ...
  go: (id, to) => {
    ...
    //     styles[style] = $parts.parent(`.${style}`);
    // });
...
document.querySelectorAll(slider.nav.item).onclick = (evt) => {
  const target = evt.target;
  const parent = target.findParent(slider.nav.parent);
  slider.go(
    parent.getAttribute(slider.attr.id),
    target.getAttribute(slider.attr.to)
  );
};

document.querySelectorAll(`.arrows > ${slider.nav.parent}`).onclick = (evt) => {
  const target = evt.target;
  const child = target.findChild(slider.nav.item);
  slider.go(
    target.getAttribute(slider.attr.id),
    child.getAttribute(slider.attr.to)
  );
};

window.addEventListener("load", () => slider.setParentHeight());
window.addEventListener("resize", () => slider.setParentHeight());
